...
def sampling_dashboard(request: Request, db: Session = Depends(get_db)):
    ...
    # JANGAN generate ulang, cukup baca file yang sudah ada
    time_ranges = ['00-06', '06-09', '09-12', '12-15', '15-18', '18-00']
    graphs = []
    
    for tr in time_ranges:
        filename = f"static/grafik/sampling_{tr.replace('-', '_')}.png"
        if os.path.exists(filename):
            # Ambil data best point dari database
            best = db.query(models.SamplingAnalysis).filter(
                models.SamplingAnalysis.time_range == tr,
                models.SamplingAnalysis.is_most_representative == True
            ).first()
            
            graphs.append({
                "time_range": tr,
                "file_url": f"/static/grafik/sampling_{tr.replace('-', '_')}.png",
                "global_avg": best.global_avg_delta if best else 0,
                "best_point": best.point_id if best else None,
                "best_deviation": best.deviation_from_global if best else None
            })
...
def analyze_sampling(db: Session = Depends(get_db)):
    ...
    if len(all_data) < 3:
        raise HTTPException(status_code=400, detail="Data kurang. Minimal 3 titik untuk analisis")
...
def visualize_sampling(db: Session = Depends(get_db)):
    """
    Generate grafik (per time_range) dari data sampling analysis
    ...
    # Kelompokkan berdasarkan time_range
    time_ranges = ['06-09', '09-12', '12-15', '15-18', '18-00', '00-06']
    ...
    for time_range in time_ranges:
        ...
        # UBAH: Minimal 3 data (bukan 6)
        if len(range_data) < 3:
            continue
        
        # Urutkan berdasarkan point_id
        ...
        # Tambahkan deviation di bawah bar
        ...
        if best_point:
            plt.figtext(0.5, 0.02, 
                       f'BEST Titik Terbaik: {best_point.point_id} (deviasi = {best_point.deviation_from_global:.2f}C) | '
    ...
    if not generated_files:
        raise HTTPException(status_code=404, detail="Tidak ada grafik yang dihasilkan. Pastikan data analisis tersedia.")
